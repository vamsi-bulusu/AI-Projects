...
def compute_average_distance(environment, start, end):
    neighbours = [end]
    average_distance = 0
    for neighbour in environment.graph[end]:
        neighbours.append(neighbour)
    for neighbour in neighbours:
        average_distance += environment.distance[start][neighbour]
    average_distance /= len(neighbours)
    return average_distance


def move_agent(environment, predator_pos, prey_pos):
    next_pos = environment.agent_pos
    curr_distance_from_prey = compute_average_distance(environment, next_pos, prey_pos)
    curr_distance_from_predator = compute_average_distance(environment, next_pos, predator_pos)
    options = [[] for _ in range(6)]

    for neighbour in environment.graph.get(environment.agent_pos):
        neighbour_distance_from_prey = compute_average_distance(environment, neighbour, prey_pos)
        neighbour_distance_from_predator = compute_average_distance(environment, neighbour, predator_pos)
...
if __name__ == "__main__":
    ...
    for num in range(NG):
        ...
        for run in range(runs):
            ...
            result, survey_prey_counter, survey_predator_counter, total_steps = agent_nine(env)
