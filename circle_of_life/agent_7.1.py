...
def update_belief_predator(environment, belief_predator):
    ...
    # Todo - Belief update for Predator
...
survey_node -> false { with 0.1 probability it can give false reading }
survey_node -> true { survey_node reading is true }
 
# Prob(Predator or Prey at node i) =  Prob(Predator or Prey at node i, Predator or Prey not at surveyed node) 
                                    + Prob(Predator or Prey at node i, Predator or Prey at surveyed node)

P(sr -> false) = 0.1
P(sr -> true) = 1 - P(sr -> false) = 0.9  
Prob(P at X) = Prob(P at X, SR -> TRUE) + Prob(P at X, SR -> False)
             =  1 + (Prob(P at X | SR -> False) * Prob(SR -> False)
             =  1 + 0.1 * (Prob(P at X | SR -> False))
             =  
...
if __name__ == "__main__":
    ...
    for num in range(NG):
        ...
        for run in range(runs):
            ...
            result, survey_prey_counter, survey_predator_counter, total_steps = agent_seven(env)
    ...
    print('Avg(Agent Success Rate): ', average_rate[0] / runs, '%', '|', 'Avg(Agent Failure rate)',
          average_rate[1] / runs, '%' '|', 'Avg(Agent Timeout rate)', average_rate[2] / runs, '%' '|', 'Avg(Agents Correct Prey Survey)', (average_rate[3] * 100) / average_rate[5], '%', '|', 'Avg(Agents Correct Predator Survey)', (average_rate[4] * 100) / average_rate[5], '%')
